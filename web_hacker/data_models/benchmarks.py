        ...
        if self.generated_routine is not None:
            ...
            if self.deterministic_tests:
                ...
                for test in self.deterministic_tests:
                    ...
                    self._print_if_verbose(f"  {'✓' if passed else '✗'} {test.name}")
            ...
            if run_llm_tests and self.llm_tests:
                ...
                for test in self.llm_tests:
                    ...
                    self._print_if_verbose(f"  {'✓' if passed else '✗'} {test.name}: {result.score:.2f} (threshold: {test.passing_threshold})")
                    if result.rationale:
                        self._print_if_verbose(f"    Rationale: {result.rationale[:100]}...")
