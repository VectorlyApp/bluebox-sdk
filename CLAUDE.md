...
### Quickstart
- `python quickstart.py` - Run the interactive quickstart script (requires OPENAI_API_KEY)
...
- Write tests first for easily verifiable changes
...
- Verify Chrome is running: `curl http://127.0.0.1:9222/json/version`
- Check `--host` and `--port` flags match Chrome launch args
- Ensure Chrome is launched with `--remote-debugging-port=9222`

### Routine Execution Issues
- Check placeholder escaping (string params need `\"{{param}}\"`)
- Verify all parameters are provided
- Check session storage keys are correct
- Review operation sequence in routine.json

### Discovery Issues
- Ensure `--task` description matches what was performed during monitoring
- Verify network transactions were captured (check `cdp_captures/network/`)
- Task description should be specific but not too narrow

## Additional Notes

- The codebase uses Pydantic v2 for data validation
- All operations are defined as union types in `RoutineOperationUnion`
- CDP communication uses WebSockets via `websockets` library
- Network monitoring captures HAR format for compatibility
- Routines can be deployed to console.vectorly.app for production use
